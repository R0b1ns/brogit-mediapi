...
from app.modules.gmrender_resurrect import module_bp

@module_bp.route('/api/config', methods=['GET'])
def config():
    return jsonify(Backend().gmrender_resurrect.get())
...
def install():
    ...
    if enabled:
        try:
            Backend().gmrender_resurrect.install(confirm=True)
        except Exception as e:
            return jsonify(error=f"Installation failed: {str(e)}"), 500
    else:
        try:
            Backend().gmrender_resurrect.uninstall(confirm=True)
        except Exception as e:
